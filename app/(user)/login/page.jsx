...
export default function LoginPage() {
  ...
  const lettersRef = useRef([]); // Ref for individual letters

  const title = "WorkSpera";
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.fromTo(containerRef.current, { opacity: 0 }, { opacity: 1, duration: 1 })
        ...
        // 4. Stagger remaining form items
  ...
  return (
    <div ref={containerRef} className="flex h-screen bg-[#020617] text-slate-200 overflow-hidden font-sans selection:bg-indigo-500/30 relative" style={{ backgroundImage: 'radial-gradient(ellipse at top, #0f172a, #020617, #000000)' }}>
      <style jsx global>{`
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div ref={el => backgroundCirclesRef.current[0] = el} className="absolute top-[10%] left-[10%] w-[400px] h-[400px] bg-indigo-600/20 rounded-full blur-[120px]" />
        <div ref={el => backgroundCirclesRef.current[1] = el} className="absolute bottom-[10%] right-[10%] w-[350px] h-[350px] bg-purple-600/20 rounded-full blur-[100px]" />
      </div>

      <div className="z-10 w-full flex items-center justify-center p-4">
        <div ref={cardRef} className="glass-card w-full max-w-[440px] rounded-[2rem] p-8 md:p-12" style={{ perspective: '1000px' }}>

          {/* Animated Letter Header */}
          <div className="text-center mb-10 overflow-hidden py-2">
            <h1 className="text-4xl md:text-5xl font-black tracking-tighter flex justify-center items-center">
              {title.split('').map((char, index) => (
                <span
                  key={index}
                  ref={addLetterToRefs}
                  className="inline-block bg-gradient-to-br from-white via-slate-300 to-slate-500 bg-clip-text text-transparent transform-gpu"
                >
                  {char === " " ? "\u00A0" : char}
                </span>
              ))}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div ref={addToRefs} className="space-y-2">
              ...
              <input
                ...
                className="w-full px-5 py-4 bg-slate-950/50 border border-slate-800 rounded-2xl focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-all duration-300 text-slate-200"
                required
              />
            </div>
            ...
            >
              <span className="relative z-10">{isLoading ? 'Processing...' : 'Sign In'}</span>
          ...
          <div ref={addToRefs}>
            <button
              ...
              className="w-full flex items-center justify-center gap-3 py-4 bg-slate-900/50 border border-slate-800 rounded-2xl hover:bg-slate-800 transition-all duration-300 text-sm font-semibold"
            >
              {isGoogleLoading ? (
          ...
          <div ref={addToRefs} className="mt-10 text-center">
            <p className="text-sm text-slate-500">
              New here?{' '}
              <Link href="/register" className="text-white font-bold hover:underline underline-offset-4">Create account</Link>
            </p>
          </div>
        </div>
      </div>
